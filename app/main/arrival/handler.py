...
def waiting_for_arrival(app: Flask, branch: str):
    collection = []
    ...
    schedule_client = GoogleAPIClient(
        book_id=GoogleSheets.ArrivalSM.value,
        sheet_title='Schedule'
    )
    schedule_client.update_collection(
        collection=schedule_client.update_arrival_schedule(source_sheet_title='Arrival'),
        unique_key='Date'
    )
    return Response(status=204)

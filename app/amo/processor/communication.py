...
class CommunicationBase:
    ...
    def __init__(
        ...
        Args:
            ...
            pipelines_dict: словарь воронок
    ...
    def __build_reaction_data(self, lead: Dict) -> Dict:
        ...
        for note in sorted(lead.get('notes') or [], key=lambda x: x['created_at']):
            ...
            user = (self.users_dict.get(created_by) or {}).get('name')
            key = None
            extra_key = None
        ...
        for event in sorted(lead.get('events') or [], key=lambda x: x['created_at']):
            ...
            if _type == AmoEvent.StageChanged.value:
                ...
                pipeline = self.pipelines_dict.get(pipeline_id) or {}
                status = (pipeline.get('statuses') or {}).get(status_id)
        ...
        for task in lead.get('tasks') or []:
            ...
            user = (self.users_dict.get(responsible_user_id) or {}).get('name')
        ...
        reaction[self.Field.CreationSource.value] = self.__get_creation_source(
            ...
            input_field=lead.get(self.lead.InputField.Key),
    ...
    def __mixin_reaction_to_lead(self, lead: Dict, reaction_time: Dict):
        ...
        lead[self.lead.CreationSource.Key] = reaction_time.get(self.Field.CreationSource.value)
        # print(lead['id'], fastest_reaction, reaction_time[self.Field.OnDuty.value], )
        # способ самой быстрой реакции
        lead[self.lead.ReactionType.Key] = fastest_reaction
        lead[self.lead.FirstSuccessfulIncomingCall.Key] = reaction_time[self.Field.FirstSuccessfulIncomingCall.value]
        # время самой быстрой реакции
        if reaction_time[self.Field.OnDuty.value]:
            lead[self.lead.OnDuty.Key] = 'duty time'
            lead[self.lead.ReactionTimeOnDuty.Key] = fastest_time or ''
            # любая реакция
            key_reaction_missed = self.lead.ReactionTimeOnDutyMissed.Key
            key_reaction_less_300 = self.lead.ReactionTimeOnDutyLess300.Key
            key_reaction_greater_300 = self.lead.ReactionTimeOnDutyGreater300.Key
            # звонки
            key_call_missed = self.lead.CallbackTimeOnDutyMissed.Key
            key_call_less_300 = self.lead.CallbackTimeOnDutyLess300.Key
            key_call_greater_300 = self.lead.CallbackTimeOnDutyGreater300.Key
        else:
            lead[self.lead.OnDuty.Key] = 'off time'
            lead[self.lead.ReactionTimeOffDuty.Key] = fastest_time or ''
            # любая реакция
            key_reaction_missed = self.lead.ReactionTimeOffDutyMissed.Key
            key_reaction_less_300 = self.lead.ReactionTimeOffDutyLess300.Key
            key_reaction_greater_300 = self.lead.ReactionTimeOffDutyGreater300.Key
            # звонки
            key_call_missed = self.lead.CallbackTimeOffDutyMissed.Key
            key_call_less_300 = self.lead.CallbackTimeOffDutyLess300.Key
            key_call_greater_300 = self.lead.CallbackTimeOffDutyGreater300.Key
        # все типы реакций
        if fastest_time is None:
            lead[key_reaction_missed] = 1
            lead[self.lead.ReactionTimeMissed.Key] = 1
        elif fastest_time <= 300:
            lead[key_reaction_less_300] = 1
            lead[self.lead.ReactionTimeLess300.Key] = 1
        else:
            lead[key_reaction_greater_300] = 1
            lead[self.lead.ReactionTimeGreater300.Key] = 1
        # только звонки
        if not fastest_call:
            lead[key_call_missed] = 1
            lead[self.lead.CallbackTimeMissed.Key] = 1
        elif fastest_call <= 300:
            lead[key_call_less_300] = 1
            lead[self.lead.CallbackTimeLess300.Key] = 1
        else:
            lead[key_call_greater_300] = 1
            lead[self.lead.CallbackTimeGreater300.Key] = 1
